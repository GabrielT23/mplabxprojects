...
unsigned int Valor_AD(unsigned char entrada) {
    ...
    while (!ADCON0bits.GO_DONE); // Aguarda enquanto o valor é convertido
...
void main(void){
 ...
 TRISD = 0x00;
 inicializa_lcd();
 // Inicialização do PORTA (RA0/AN0 RA1/AN1 como entrada)
 PORTA = 0;
 ...
 while (1) {
     //posiciona o cursor em uma posição do lcd
    caracter_inicio(1,1);
    printf("V1:=%.2f V",Tensao(1)); //Exibe o valor da tensão de entrada1
    
    caracter_inicio(2,1);
    printf("V2:=%.2f V",Tensao(0));//Exibe a valor da tensão de entrada2
    __delay_ms(50);  
 }
